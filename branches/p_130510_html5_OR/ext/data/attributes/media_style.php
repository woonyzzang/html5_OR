<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="utf-8">
<title>media 속성 (style) : HTML5 Open Reference Guide</title>
<link rel="stylesheet" href="../../resources/css/ext-all.css">
<link rel="stylesheet" href="../../resources/css/base.css">
<script src="http://code.jquery.com/jquery-1.9.1.min.js"></script>
<script src="http://code.jquery.com/jquery-migrate-1.1.1.min.js"></script>
<script src="../../ext-all-debug.js"></script>
<script src="../../locale/ext-lang-ko.js"></script>
<script>

</script>
</head>
<body>
<article id="content_desc">
	<!-- 태그 설명 -->
	<section>
		<h2 class="s_tit">media 속성 (style)</h2>

		<dl class="definition">
		<dt>정의</dt>
		<dd>media 속성은 style 요소에서 스타일에 적용될 매체를 나타냅니다.</dd>
		</dl>

		<h3 class="s_tit">속성 값</h3>
		<p>값은 유효한 미디어 쿼리여야 합니다. 속성이 생략되었다면, 기본값은 all입니다.</p>
	</section>
	<!-- //태그 설명 -->
	<!-- 참고 -->
	<section>
		<h3 class="s_tit">참고 API</h3>
		<ul class="api_ist">
		<li><a href="http://www.w3.org/TR/html5/semantics.html#attr-style-media" target="_blank">최신 명세</a></li>
		<li><a href="http://www.w3.org/TR/2011/WD-html5-20110113/common-microsyntaxes.html#valid-media-query" target="_blank">valid media query</a></li>
		</ul>
	</section>
	<!-- //참고 -->
</article>
</body>
</html>